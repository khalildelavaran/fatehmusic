...
function findCourseForTopic(topic, courses = [], title = "") {
  ...
  const sameInstrument = courses.filter((course) => normalize(course?.instrument || "") === normalize(bestCourse.instrument || ""));
...
function buildCandidateBrief(candidate, courses = [], siteUrl) {
  const baseUrl = normalizeBaseUrl(siteUrl); const explicitCourse = candidate.relatedCourseSlug ? courses.find((item) => item?.slug === candidate.relatedCourseSlug) || null; const topic = resolveCandidateTopic(candidate, explicitCourse); if (!topic) return null; const intent = candidate.intent || "informational"; const isLocal = candidate.modifierType === "local_shushtar" || hasLocalSignal(candidate.title) || isShushtarTopic(topic); const course = explicitCourse || (topic.slug === "shushtar" ? null : findCourseForTopic(topic, courses, candidate.title)); const targetEntity = buildTargetEntity(topic, course, isLocal, baseUrl);

...
function isShushtarTopic(topic) {
  return topic?.slug === "shushtar" || normalize(topic?.name).includes("شوشتر");
}

function localTopicName(topic) {
  return isShushtarTopic(topic) ? "آموزش موسیقی" : (topic?.name || "آموزش موسیقی");
}

function hasLocalSignal(value) {
  return normalize(value).includes("شوشتر");
}
  ...
  const specific = TOPICS
    ...
    .map((topic) => ({
      ...
      score: (topic.aliases || []).reduce(
        (sum, alias) => normalized.includes(normalize(alias))
          ? sum + normalize(alias).length
          : sum,
        0
      )
    }))
  ...
  const ranked = courses
    .map((course) => {
      const haystack = normalize(
        [course?.slug, course?.title, course?.instrument, course?.description]
          .filter(Boolean)
          .join(" | ")
      );
      const exact = aliases.find(
        (alias) => haystack === alias || haystack.split(" ").includes(alias)
      );
      const score = exact
        ? 1000 + exact.length
        : aliases.reduce(
            (sum, alias) => sum + (haystack.includes(alias) ? alias.length : 0),
            0
          );
      return { course, score };
    })
...
function buildTitle(topic, intent, course) {
  ...
  return course
    ? `${courseName}: ${INTENT_SUFFIX[intent]}`
    : `${name}: ${INTENT_SUFFIX[intent]} یادگیری و تمرین`;
}
...
function buildTargetEntity(topic, course, isLocal, baseUrl) {
  ...
  if (isLocal || isShushtarTopic(topic)) {
    return {
      type: "Place",
      id: `${baseUrl}/#place-shushtar`,
      name: "آموزش موسیقی در شوشتر",
      url: `${baseUrl}/locations/shushtar`
    };
  }
...
function canonicalScope(topic, isLocal = false) {
...
function canonicalSlug(topic, isLocal = false, course = null) {
  if (isLocal || isShushtarTopic(topic)) {
    ...
    return `${base}${course?.slug ? `-${course.slug}` : ""}-shushtar`;
  }

  return course?.slug || topic.slug;
}

function canonicalContentAngle(item) {
  if (item.modifierType === "local_shushtar" || item.scope === "shushtar") {
    return "local_shushtar";
  }
...
function canonicalAssetKey(item) {
  ...
  const local = Boolean(
    item.scope === "shushtar" ||
      item.modifierType === "local_shushtar" ||
  ...
  return [
    ...
    canonicalScope(topic, local),
    courseKey,
    canonicalContentAngle({
      ...item,
      scope: local ? "shushtar" : item.scope
    }),
    audience,
    level
  ].join("|");
...
export function areIntentsCompatible(a, b) {
  return Boolean(
    COMPATIBLE_INTENTS[a]?.has(b) && COMPATIBLE_INTENTS[b]?.has(a)
  );
}

function choosePrimaryIntent(intents = []) {
  return (
    [...new Set(intents)]
      .sort((a, b) => (INTENT_PRIORITY[b] ?? 0) - (INTENT_PRIORITY[a] ?? 0))[0] ||
    "informational"
  );
}

function mergeIntents(a = [], b = []) {
  return [...new Set([...a, ...b])].filter((intent) => INTENTS.includes(intent));
}

function mergeQueryAngles(topic, intents, course) {
  return Object.freeze(
    [...new Set(intents.flatMap((intent) => buildQueryAngles(topic, intent, course)))].slice(
      0,
      15
    )
  );
...
function makeCourseRef(course, baseUrl) {
  return course
    ? Object.freeze({
        slug: course.slug,
        title: course.title,
...
function buildBrief(gap, courses = [], siteUrl) {
  ...
  const isLocal =
    gap.scope === "shushtar" || intent === "local" || isShushtarTopic(topic);
  const course = gap.courseSlug
    ? courses.find((item) => item?.slug === gap.courseSlug) || null
    : isLocal && isShushtarTopic(topic)
      ? null
      : findCourseForTopic(topic, courses);
  ...
  return Object.freeze({
    ...
    isLocal,
    scope: gap.scope || canonicalScope(topic, isLocal),
    title: buildTitle(topic, intent, course),
    suggestedSlug: canonicalSlug(topic, isLocal, course),
    targetEntity,
    course: makeCourseRef(course, baseUrl),
    courseSlug: gap.courseSlug || course?.slug || null,
    priority: buildPriority(intent, articleCount, course, isLocal),
    articleCount,
    existingArticleSlugs: gap.articleSlugs || [],
    rationale:
      action === "OPTIMIZE_EXISTING"
        ? `intent «${intent}» برای خوشه «${topic.name}» ناقص است؛ محتوای موجود باید برای پوشش این intent تقویت شود.`
    ...
    modifierType: isLocal ? "local_shushtar" : null,
    audience: "",
    level: ""
  });
}

function buildContentStrategyFromGaps(gaps = [], courses = [], siteUrl) {
  return gaps.flatMap((gap) =>
    (gap.missingIntents || [])
      .map((intent) => buildBrief({ ...gap, missingIntents: [intent] }, courses, siteUrl))
      .filter(Boolean)
  );
}
...
function buildCandidateBrief(candidate, courses = [], siteUrl) {
  const baseUrl = normalizeBaseUrl(siteUrl);
  const explicitCourse = candidate.relatedCourseSlug
    ? courses.find((item) => item?.slug === candidate.relatedCourseSlug) || null
    : null;
  ...
  const isLocal =
    ...
    hasLocalSignal(candidate.title) ||
    isShushtarTopic(topic);
  const course =
    explicitCourse ||
    (topic.slug === "shushtar" ? null : findCourseForTopic(topic, courses));
  const targetEntity = buildTargetEntity(topic, course, isLocal, baseUrl);
  ...
  return Object.freeze({
    ...
    rationale:
      candidate.reasoning || "این موضوع توسط موتور تولید موضوعات کشف و امتیازدهی شده است.",
    queryAngles: buildQueryAngles(topic, intent, course),
    recommendedLinks: buildRecommendedLinks(targetEntity, baseUrl),
...
function mergeOpportunity(candidate, gap, siteUrl) {
  ...
  return Object.freeze({
    ...
    searchIntent: choosePrimaryIntent(searchIntents),
    searchIntents,
    isLocal,
    scope:
      gap.scope ||
      candidate.scope ||
      canonicalScope(topic, isLocal),
...
function mergeCompatibleOpportunities(items, siteUrl) {
  ...
  for (const item of items) {
    ...
    const compatible = currentIntents.every((a) =>
      incomingIntents.some((b) => areIntentsCompatible(a, b))
    );
    ...
    const topic = findTopic(preferred.topic);
    const preferredCourse = preferred.course || current.course || item.course || null;
    const local = Boolean(current.isLocal || item.isLocal);
    ...
    groups.set(
      ...
      Object.freeze({
        ...
        queryAngles: topic
          ? mergeQueryAngles(topic, mergedIntents, preferredCourse)
          : preferred.queryAngles,
        existingArticleSlugs: [
          ...new Set([
            ...(current.existingArticleSlugs || []),
            ...(item.existingArticleSlugs || [])
          ])
        ],
        articleCount: Math.max(
          Number(current.articleCount) || 0,
          Number(item.articleCount) || 0
        ),
        action:
          [current.action, item.action].includes("OPTIMIZE_EXISTING")
            ? "OPTIMIZE_EXISTING"
            : preferred.action,
        ...
        scope:
          ...
          item.scope ||
          canonicalScope(topic, local),
        course: makeCourseRef(preferredCourse, baseUrl),
        targetEntity: topic
          ? buildTargetEntity(topic, preferredCourse, local, baseUrl)
          : preferred.targetEntity,
        suggestedSlug: topic
          ? canonicalSlug(topic, local, preferredCourse)
          : preferred.suggestedSlug,
        courseSlug:
          preferred.courseSlug ||
          current.courseSlug ||
          item.courseSlug ||
          preferredCourse?.slug ||
          null,
        audience: preferred.audience || current.audience || item.audience || "",
        level: preferred.level || current.level || item.level || "",
...
export function buildUnifiedContentOpportunities({
  gaps = [],
  topicCandidates = [],
  courses = [],
  siteUrl
  ...
  const candidateItems = topicCandidates
    .map((candidate) =>
      candidate?.title
        ? buildCandidateBrief(candidate, courses, siteUrl)
        : null
    )
    .filter(Boolean);
  ...
  const mergedByKey = new Map(
    mergedCandidates.map((item) => [canonicalAssetKey(item), item])
  );
  const unmatchedGaps = [];

  for (const gap of buildContentStrategyFromGaps(gaps, courses, siteUrl)) {
    const key = canonicalAssetKey(gap);
    const candidate = mergedByKey.get(key);

    if (candidate) {
      const candidateIntents = candidate.searchIntents || [candidate.searchIntent];
      const gapIntents = gap.searchIntents || [gap.searchIntent];
      const compatible = candidateIntents.every((a) =>
        gapIntents.some((b) => areIntentsCompatible(a, b))
      );

      if (compatible) {
        mergedByKey.set(key, mergeOpportunity(candidate, gap, siteUrl));
        continue;
      }
    }
  ...
  const opportunities = mergeCompatibleOpportunities(
    [...mergedByKey.values(), ...unmatchedGaps],
    siteUrl
  ).sort(
    (a, b) =>
...
export function buildContentStrategy(gaps = [], courses = [], { siteUrl } = {}) {
  const briefs = mergeCompatibleOpportunities(
    buildContentStrategyFromGaps(gaps, courses, siteUrl),
    siteUrl
  );

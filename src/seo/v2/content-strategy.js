// This file intentionally keeps intent as metadata on a canonical content asset.
// Local/commercial/transactional service intents are deduplicated; informational and
// navigational intents remain separate unless an explicit compatibility rule is added.
...
export function buildUnifiedContentOpportunities({ gaps = [], topicCandidates = [], courses = [], siteUrl } = {}) {
  ...
  const gapsItems = buildContentStrategyFromGaps(gaps, courses, siteUrl);
  const mergedByKey = new Map(mergedCandidates.map((item) => [canonicalAssetKey(item), item]));
  for (const gap of gapsItems) {
    const key = canonicalAssetKey(gap);
    const candidate = mergedByKey.get(key);
    const merged = candidate ? mergeOpportunity(candidate, gap, siteUrl) : gap;
    const current = mergedByKey.get(key);
    mergedByKey.set(key, current && current !== candidate ? mergeCompatibleOpportunities([current, merged])[0] : merged);
  }
  const opportunities = mergeCompatibleOpportunities([...mergedByKey.values()]).sort((a, b) => b.priority - a.priority || a.title.localeCompare(b.title, "fa"));

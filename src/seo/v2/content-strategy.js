...
function canonicalSlug(topic, isLocal = false, course = null) {
  if (isLocal || isShushtarTopic(topic)) {
    // Course slugs already contain the subject prefix, e.g. guitar-course.
...
function buildBrief(gap, courses = [], siteUrl) {
  const topic = findTopic(gap.topic); const intent = gap.missingIntents?.[0]; if (!topic || !intent) return null; const baseUrl = normalizeBaseUrl(siteUrl); const isLocal = gap.scope === "shushtar" || intent === "local" || isShushtarTopic(topic); const course = gap.courseSlug ? courses.find((item) => item?.slug === gap.courseSlug) || null : isLocal && isShushtarTopic(topic) ? null : findCourseForTopic(topic, courses); const articleCount = Number(gap.articleCount) || 0; const action = articleCount > 0 ? "OPTIMIZE_EXISTING" : "NEW_CONTENT"; const targetEntity = buildTargetEntity(topic, course, isLocal, baseUrl);
...
function buildCandidateBrief(candidate, courses = [], siteUrl) {
  const baseUrl = normalizeBaseUrl(siteUrl); const explicitCourse = candidate.relatedCourseSlug ? courses.find((item) => item?.slug === candidate.relatedCourseSlug) || null : null; const topic = resolveCandidateTopic(candidate, explicitCourse); if (!topic) return null; const intent = candidate.intent || "informational"; const isLocal = candidate.modifierType === "local_shushtar" || hasLocalSignal(candidate.title) || isShushtarTopic(topic); const course = explicitCourse || (topic.slug === "shushtar" ? null : findCourseForTopic(topic, courses)); const targetEntity = buildTargetEntity(topic, course, isLocal, baseUrl);

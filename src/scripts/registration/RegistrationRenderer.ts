...
import { buildContract } from "./ContractTemplates";
import {
  classModeLabel,
  escapeHtml,
  formatMobileDisplay,
...
class RegistrationRenderer {
  updateProgress(step:string){const rail=document.querySelector<HTMLElement>("[data-progress]");if(!rail)return;const isWizardStep=(PROGRESS_STEPS as readonly string[]).includes(step);rail.hidden=!isWizardStep&&step!=="success";const currentIndex=step==="success"?PROGRESS_STEPS.length:PROGRESS_STEPS.indexOf(step as (typeof PROGRESS_STEPS)[number]);rail.querySelectorAll<HTMLElement>("[data-progress-step]").forEach((node)=>{const nodeStep=node.dataset.progressStep as (typeof PROGRESS_STEPS)[number];const nodeIndex=PROGRESS_STEPS.indexOf(nodeStep);node.classList.remove("is-active","is-complete");if(nodeIndex<currentIndex)node.classList.add("is-complete");else if(nodeIndex===currentIndex)node.classList.add("is-active");});}
  renderSelectionSummary(state:RegistrationState){const el=document.querySelector<HTMLElement>("[data-selection-summary]");if(!el)return;const{instrument,instructor,schedule}=state.selection;const chips:string[]=[];if(instrument.title)chips.push(this.summaryChip("ساز",instrument.title));if(instructor.name)chips.push(this.summaryChip("استاد",instructor.name,instructor.auto));if(schedule.weekday)chips.push(this.summaryChip("زمان",this.formatSchedule(schedule)??"",schedule.auto));if(!chips.length){el.hidden=true;el.innerHTML="";return;}el.hidden=false;el.innerHTML=chips.join("");}
  ...
  filterInstrumentsByInstructor(instructorId:number){document.querySelectorAll<HTMLElement>('[data-step="instrument"] [data-instrument-id]').forEach((card)=>{const ids=(card.dataset.instructorIds??"").split(",").filter(Boolean).map(Number);card.hidden=!ids.includes(instructorId);});}
  ...
  updateSuccess(state:RegistrationState){const{instrument,instructor,schedule}=state.selection;this.setText("success-tracking-code",state.trackingCode);this.setText("success-instrument",instrument.title);this.setText("success-instructor",instructor.name);this.setText("success-schedule",this.formatSchedule(schedule));this.renderContactLinks();this.renderContract(state);if(instrument.type)this.renderMaterials(instrument.type);}
  private renderContract(state:RegistrationState){const container=document.querySelector<HTMLElement>("[data-contract-container]");if(!container)return;const contract=buildContract(state);if(!contract){container.hidden=true;return;}container.hidden=false;container.innerHTML=`${contract.blocks.map((block)=>`<div class="contract-article">${block.heading?`<h4>${escapeHtml(block.heading)}</h4>`:""}${block.paragraphs.map((p)=>`<p>${escapeHtml(p)}</p>`).join("")}</div>`).join("")}<div class="contract-signature"><div class="contract-signature-col"><span>محل امضاء هنرجو (یا ولی و سرپرست)</span><span>نام و نام خانوادگی: ${escapeHtml(contract.signature.studentName)}</span><span>تاریخ: ${escapeHtml(contract.signature.date)}</span></div><div class="contract-signature-col"><span>محل امضاء آموزشگاه</span></div></div>`;}
  private renderMaterials(instrumentType:string){const container=document.querySelector<HTMLElement>("[data-materials-container]");if(!container)return;const materials=getMaterialsByInstrument(instrumentType);const renderGroup=(title:string,items:any[],optional:boolean)=>{if(!items.length)return"";return `<section class="materials-group"><h4>${title}</h4><div class="materials-grid">${items.map((item)=>`<article class="material-card${optional?" optional":""}">${item.image?`<img src="${escapeHtml(item.image)}" alt="${escapeHtml(item.title)}" loading="lazy" />`:""}<strong>${escapeHtml(item.title)}</strong>${optional?`<span class="optional-tag">اختیاری</span>`:""}</article>`).join("")}</div></section>`;};container.innerHTML=renderGroup("موارد ضروری",materials.required,false)+renderGroup("موارد اختیاری",materials.optional,true);}
  private renderContactLinks(){const container=document.querySelector<HTMLElement>("[data-contact-links]");if(!container)return;const telHref=`tel:+${contact.phones.mobile.raw}`;container.innerHTML=`<a href="${contact.social.whatsapp}" class="btn" target="_blank" rel="noopener">پیام واتساپ</a><a href="${telHref}" class="secondary-btn">تماس تلفنی</a>`;}
  private formatSchedule(schedule:RegistrationState["selection"]["schedule"]):string|null{if(!schedule?.weekday)return null;const parts=[schedule.weekday];if(schedule.sessionDuration)parts.push(`${toPersianDigits(schedule.sessionDuration)} دقیقه`);if(schedule.classroom)parts.push(`کلاس ${toPersianDigits(schedule.classroom)}`);return parts.join(" · ");}
  private translateGender(value:RegistrationState["student"]["gender"]):string{const map:Record<string,string>={male:"آقا",female:"خانم"};return map[value??""]??"-";}
  private translateInstrumentStatus(value:RegistrationState["student"]["hasInstrument"]):string{const map:Record<string,string>={yes:"ساز دارم",no:"نیاز به مشاوره خرید ساز دارم"};return map[value??""]??"-";}
  private setText(field:string,value:string|number|null|undefined){const element=document.querySelector(`[data-field-out="${field}"]`);if(element)element.textContent=value?String(value):"-";}
}

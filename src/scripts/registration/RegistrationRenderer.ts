...
class RegistrationRenderer {
  ...
  /**
   * Fills and renders the student contract (قرارداد هنرجویی) for
   * printing. See ContractTemplates.ts for exactly which fields are
   * auto-filled versus deliberately left blank.
   */
  private renderContract(state: RegistrationState) {
    const container = document.querySelector<HTMLElement>("[data-contract-container]");
    if (!container) return;
    ...
    container.innerHTML = contract.blocks.map((block) => `
        ...
        <div class="contract-signature">
          <div class="contract-signature-col">
            <span>محل امضاء هنرجو (یا ولی و سرپرست)</span>
            <span>نام و نام خانوادگی: ${escapeHtml(contract.signature.studentName)}</span>
            <span>تاریخ: ${escapeHtml(contract.signature.date)}</span>
          </div>
          <div class="contract-signature-col">

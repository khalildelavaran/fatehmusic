...
class RegistrationRenderer {
  ...
  updateSuccess(state: RegistrationState) { const { instrument, instructor, schedule } = state.selection; this.setText("success-tracking-code", state.trackingCode); this.setText("success-instrument", instrument.title); this.setText("success-instructor", instructor.name); this.setText("success-schedule", this.formatSchedule(schedule)); this.renderContactLinks(); if (instrument.type) this.renderMaterials(instrument.type); }
  private renderMaterials(instrumentType: string) { const container = document.querySelector<HTMLElement>("[data-materials-container]"); if (!container) return; const materials = getMaterialsByInstrument(instrumentType); const renderGroup = (title: string, items: any[], optional: boolean) => !items.length ? "" : `<section class="materials-group"><h4>${title}</h4><div class="materials-grid">${items.map((item) => `<article class="material-card${optional ? " optional" : ""}">${item.image ? `<img src="${escapeHtml(item.image)}" alt="${escapeHtml(item.title)}" loading="lazy" />` : ""}<strong>${escapeHtml(item.title)}</strong>${optional ? `<span class="optional-tag">اختیاری</span>` : ""}</article>`).join("")}</div></section>`; container.innerHTML = renderGroup("موارد ضروری", materials.required, false) + renderGroup("موارد اختیاری", materials.optional, true); }
  ...
  private translateGender(value: RegistrationState["student"]["gender"]): string { return ({ male: "آقا", female: "خانم" } as Record<string, string>)[value ?? ""] ?? "-"; }
  private translateInstrumentStatus(value: RegistrationState["student"]["hasInstrument"]): string { return ({ yes: "ساز دارم", no: "نیاز به مشاوره خرید ساز دارم" } as Record<string, string>)[value ?? ""] ?? "-"; }

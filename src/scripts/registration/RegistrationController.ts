...
class RegistrationController {
  ...
  private applyDeepLink(): boolean {
    ...
    if (teachableCourses.length > 1) {
      ...
      this.renderer.filterInstrumentsByInstructor(instructor.id); this.goToStep("instrument"); return true;
    }
    this.lockedInstructorId = null; return false;
  }

  private selectInstrument(element: HTMLElement) {
    const id = Number(element.dataset.instrumentId); const course = courses.find((item) => item.id === id); if (!course) return; this.applyInstrument(course);
  }
  ...
  private selectInstructor(element: HTMLElement) {
    const id = Number(element.dataset.instructorId); const instructor = instructors.find((item) => item.id === id); if (!instructor) return;
    registrationStore.selectInstructor({ id: instructor.id, name: instructor.name, auto: false }); this.updateSelectionSummary(); this.resolveScheduleForInstructor(instructor);
  }
  ...
  private selectSchedule(element: HTMLElement) {
    const id = Number(element.dataset.scheduleId); const schedule = schedules.find((item) => item.id === id); if (!schedule) return;
    registrationStore.selectSchedule({ id: schedule.id, weekday: schedule.weekday, sessionDuration: schedule.sessionDuration ?? null, classroom: schedule.classroom ?? null, classMode: schedule.classMode ?? null, auto: false }); this.updateSelectionSummary(); this.goToStep("student");
  }
  ...
  private continueFromStudent() {
    const student = this.collectStudent(); registrationStore.updateStudent(student);
    const { valid, errors } = registrationValidation.validateStudent(registrationStore.getState().student); this.showStudentErrors(errors); if (!valid) return;
    this.renderer.updateReview(registrationStore.getState()); this.goToStep("review");
  }
  private collectStudent() {
    ...
    scope?.querySelectorAll<HTMLInputElement>("[data-field]").forEach((input) => {
      const key = input.dataset.field; if (!key) return;
      if (input.type === "radio") { if (input.checked) student[key] = input.value; return; }
      if (key === "mobile") { student[key] = normalizeMobile(input.value); return; }
      if (key === "nationalCode") { student[key] = normalizeNationalCode(input.value); return; }
      student[key] = input.value;
    });
    if (student.birthYear) student.birthYear = Number(student.birthYear);
    student.age = student.birthYear ? getCurrentJalaliYear() - Number(student.birthYear) : null; return student;
  }
  private showStudentErrors(errors: string[]) {
    const box = document.querySelector<HTMLElement>('[data-step="student"] [data-form-errors]'); if (!box) return;
    if (!errors.length) { box.hidden = true; box.innerHTML = ""; return; }
    box.hidden = false; box.innerHTML = errors.map((message) => `<li>${message}</li>`).join("");
  }
  private reflectMobileValidity(input: HTMLInputElement) {
    ...
    const valid = isValidMobile(input.value); group.classList.toggle("is-valid", valid); group.classList.toggle("is-invalid", !valid);
  }
  private async submitRegistration() {
    const state = registrationStore.getState(); const validation = registrationValidation.validate(state);
    if (!validation.valid) { const box = document.querySelector<HTMLElement>('[data-step="review"] [data-form-errors]'); if (box) { box.hidden = false; box.innerHTML = validation.errors.map((message) => `<li>${message}</li>`).join(""); } return; }
    ...
    try {
      ...
      if (response.success) {
        registrationStore.setTrackingCode(response.trackingCode || ""); if (response.term) registrationStore.setTerm(response.term); registrationStore.complete();
        const completedState = registrationStore.getState(); this.renderer.updateSuccess(completedState);
        if (typeof window.fbq === "function") window.fbq("track", "Lead", { content_name: "Online Registration", content_category: completedState.selection.instrument.title });
        this.goToStep("success");
      } else { const box = document.querySelector<HTMLElement>('[data-step="review"] [data-form-errors]'); if (box) { box.hidden = false; box.innerHTML = `<li>${response.message}</li>`; } }
  ...
  /** Opens a completely isolated print document so the site's header/menu can never enter the print output. */
  private printContract() {
    const source = document.querySelector<HTMLElement>("[data-contract-container]");
    if (!source || !source.innerHTML.trim()) return;

    // Open synchronously from the user's click so popup blockers do not block it.
    const printWindow = window.open("", "_blank", "width=900,height=1100,noopener,noreferrer");
    if (!printWindow) {
      alert("لطفاً اجازه بازشدن پنجره جدید را برای چاپ قرارداد در مرورگر فعال کنید.");
      return;
    }

    const state = registrationStore.getState();
    const today = this.toPersianDigits(String(state.term?.completionDate ?? state.term?.date ?? new Date().toLocaleDateString("fa-IR")));
    const esc = (value: string) => value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\"/g, "&quot;");
    const contractHtml = source.innerHTML;

    printWindow.document.open();
    printWindow.document.write(`<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>قرارداد هنرجویی</title><link rel="preconnect" href="https://fonts.googleapis.com"><link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;500;600;700;800;900&display=swap" rel="stylesheet"><style>
      @page{size:A4 portrait;margin:0}*{box-sizing:border-box}html,body{margin:0!important;padding:0!important;width:210mm!important;height:297mm!important;background:#fff!important;color:#000!important;font-family:Vazirmatn,Tahoma,sans-serif!important}body{overflow:hidden!important}.sheet{position:relative;width:210mm;height:297mm;overflow:hidden;background:#fff;padding:32mm 15mm 25mm}.sheet::before{content:"";position:absolute;inset:5mm;border:.45mm solid #b9973e;pointer-events:none}.sheet::after{content:"";position:absolute;inset:7mm;border:.18mm solid #dfcf9b;pointer-events:none}.print-header{position:absolute;top:8mm;right:12mm;left:12mm;height:21mm;display:grid;grid-template-columns:42mm 1fr 42mm;direction:rtl;align-items:center;border-bottom:.45mm solid #b9973e}.header-logo{width:18mm!important;height:18mm!important;max-width:18mm!important;max-height:18mm!important;object-fit:contain!important;display:block!important}.header-logo-wrap{display:flex;align-items:center;justify-content:flex-start}.header-title{text-align:center;font-size:18pt;font-weight:900;color:#000;letter-spacing:0}.header-date{text-align:left;font-size:9pt;font-weight:600;color:#000;direction:rtl}.print-footer{position:absolute;right:12mm;left:12mm;bottom:6mm;height:15mm;border-top:.4mm solid #b9973e;display:grid;grid-template-columns:1.9fr 1fr 1.2fr 1.15fr;direction:rtl;gap:2mm;align-items:center;color:#000}.footer-item{min-width:0;padding:0 2mm;text-align:center;border-left:.2mm solid #d7c78f;color:#000;font-size:6.4pt;line-height:1.6}.footer-item:last-child{border-left:0}.footer-label{font-weight:800;color:#000}.footer-value{font-weight:500;color:#000}.contract-content{width:100%;height:100%;overflow:hidden;color:#000!important}.contract-content,.contract-content *{color:#000!important;-webkit-text-fill-color:#000!important;text-shadow:none!important}.contract-article{break-inside:avoid;page-break-inside:avoid}.contract-signature{break-inside:avoid;page-break-inside:avoid}.contract-content p,.contract-content span,.contract-content strong,.contract-content h1,.contract-content h2,.contract-content h3,.contract-content h4,.contract-content div{color:#000!important}
    </style></head><body><main class="sheet"><header class="print-header"><div class="header-logo-wrap"><img class="header-logo" src="/logo.png" alt="آموزشگاه موسیقی فاتح"></div><div class="header-title">قرارداد هنرجویی</div><div class="header-date">تاریخ: ${esc(today)}</div></header><section class="contract-content">${contractHtml}</section><footer class="print-footer"><div class="footer-item"><span class="footer-label">آدرس: </span><span class="footer-value">خیابان امام شرقی، پس از پاساژ مهستان، محوطه دوم پارکینگ حاج سلیمان</span></div><div class="footer-item"><span class="footer-label">تلفن: </span><span class="footer-value">۰۶۱-۳۶۲۲۱۱۷۴</span></div><div class="footer-item"><span class="footer-label">موبایل / WhatsApp: </span><span class="footer-value">۰۹۳۳-۳۱۳-۹۳۱۹</span></div><div class="footer-item"><span class="footer-label">Instagram: </span><span class="footer-value">fateh.music.academy</span></div></footer></main><script>window.addEventListener('load',()=>{setTimeout(()=>{window.focus();window.print();},450);});window.addEventListener('afterprint',()=>setTimeout(()=>window.close(),250));</script></body></html>`);

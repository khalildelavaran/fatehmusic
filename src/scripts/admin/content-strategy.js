...
const INTENT_LABELS = { informational: "آموزشی", commercial: "مقایسه‌ای", transactional: "تراکنشی", navigational: "برند", local: "محلی" };
...
async function loadTopics() {
  ...
  if (!data.success) {
    list.innerHTML = `<tr><td colspan="7" class="admin-table-empty">${data.message}</td></tr>`;
    return;
  }
  ...
  list.innerHTML = data.topics.map((topic) => {
    ...
    return `<tr>
      <td><strong title="${(topic.reasoning ?? "").replace(/"/g, "&quot;")}">${topic.title}</strong><div class="admin-table-subtext">${INTENT_LABELS[topic.intent] ?? topic.intent}</div></td>
...
list.addEventListener("click", async (event) => {
  ...
  if (status) {
    const response = await fetch("/api/admin/topics", {
      ...
      body: JSON.stringify({ id: Number(target.dataset.id), status })
    });
    if (response.status === 401) { window.location.assign("/admin/login"); return; }
    await loadTopics();
  }

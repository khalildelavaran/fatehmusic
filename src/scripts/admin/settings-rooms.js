...
(() => {
  ...
  async function responseJson(response) {
    ...
    if (!text.trim()) {
      throw new Error(`پاسخ خالی از سرور دریافت شد (${response.status})`);
    }
    try {
      return JSON.parse(text);
    } catch {
      throw new Error(`پاسخ JSON نامعتبر از سرور دریافت شد (${response.status})`);
    }
  }
  ...
  async function apiListRooms() {
    ...
    if (!response.ok || !data.success) {
      throw new Error(data.message || "دریافت اتاق‌ها با خطا مواجه شد.");
    }
  ...
  async function apiCreateRoom(payload) {
    ...
    const data = await responseJson(response);
    if (!response.ok || !data.success) {
      throw new Error(data.message || "افزودن اتاق با خطا مواجه شد.");
    }
    return data.room;
  }
  ...
  async function apiUpdateRoom(id, payload) {
    ...
    const data = await responseJson(response);
    if (!response.ok || !data.success) {
      throw new Error(data.message || "ویرایش اتاق با خطا مواجه شد.");
    }
    return data.room;
  }
  ...
  async function apiDeleteRoom(id) {
    const response = await fetch(
      `/api/admin/rooms?id=${encodeURIComponent(id)}`,
      {
        method: "DELETE",
        credentials: "same-origin",
        headers: { Accept: "application/json" },
    ...
    const data = await responseJson(response);
    if (!response.ok || !data.success) {
      throw new Error(data.message || "حذف اتاق با خطا مواجه شد.");
    }
    return data;
  }
  ...
  function renderList() {
    ...
    if (state.loading) {
      els.list.innerHTML = `<div class="empty-note">در حال بارگذاری…</div>`;
      return;
    }

    if (!state.rooms.length) {
      els.list.innerHTML = `<div class="empty-note">هنوز اتاقی ثبت نشده است.</div>`;
      return;
    }
  ...
  async function load() {
    ...
    try {
      ...
      state.message = err instanceof Error
        ? err.message
        : "دریافت اتاق‌ها با خطا مواجه شد.";
  ...
  root.addEventListener("click", (event) => {
    ...
    if (action === "edit") {
      state.editingId = Number(actionEl.dataset.roomId);
      state.message = null;
      render();
      ...
      toggleStatus(Number(actionEl.dataset.roomId), actionEl.dataset.next);
    } else if (action === "delete") {
      deleteRoom(Number(actionEl.dataset.roomId));
    }
  });

  // The main #roomForm has no data-action attribute, so it must be handled
  // directly from event.target. Edit forms use data-action="save-edit".
  root.addEventListener("submit", (event) => {
    const submittedForm = event.target;

    if (submittedForm === els.form) {
      event.preventDefault();
      createRoom(submittedForm);
      return;
    }

    const actionForm = submittedForm.closest("[data-action]");
    if (!actionForm || !root.contains(actionForm)) return;

    if (actionForm.dataset.action === "save-edit") {
      event.preventDefault();
      saveEdit(actionForm);
    }
  });
  ...
  async function createRoom(form) {
    ...
    try {
      ...
      state.message = "اتاق با موفقیت افزوده شد.";
      state.messageType = "success";
      await load();
    } catch (err) {
      state.message = err instanceof Error
        ? err.message
        : "افزودن اتاق با خطا مواجه شد.";
      state.messageType = "error";
      render();
    } finally {
      state.saving = false;
  ...
  async function saveEdit(form) {
    ...
    try {
      ...
      state.editingId = null;
      state.message = "تغییرات ذخیره شد.";
      state.messageType = "success";
      await load();
    } catch (err) {
      state.message = err instanceof Error
        ? err.message
        : "ویرایش اتاق با خطا مواجه شد.";
      state.messageType = "error";
      render();
    } finally {
      state.saving = false;
  ...
  async function toggleStatus(id, next) {
    ...
    try {
      ...
      state.message = next === "active" ? "اتاق فعال شد." : "اتاق غیرفعال شد.";
      state.messageType = "success";
      await load();
    } catch (err) {
      state.message = err instanceof Error

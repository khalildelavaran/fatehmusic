...
async function loadRegistrations() {
  const response = await fetch("/api/admin/registrations", { headers: headers() });
...
body.addEventListener("change", async (event) => {
  ...
  const response = await fetch("/api/admin/registrations", { method: "PATCH", headers: headers(), body: JSON.stringify({ id: Number(target.dataset.id), status: target.value }) });
  const data = await response.json();

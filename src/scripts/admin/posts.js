...
form?.addEventListener("submit", async (event) => {
  ...
  const response = await fetch("/api/admin/posts", { method: "POST", headers: jsonHeaders, credentials: "same-origin", body: JSON.stringify(payload) });
  if (await handleUnauthorized(response)) return;
  const data = await response.json();
...
list?.addEventListener("click", async (event) => {
  ...
  if (deleteId && confirm("این نوشته حذف شود؟")) {
    const response = await fetch("/api/admin/posts", { method: "DELETE", headers: jsonHeaders, credentials: "same-origin", body: JSON.stringify({ id: Number(deleteId) }) });
    if (await handleUnauthorized(response)) return;
    const data = await response.json();

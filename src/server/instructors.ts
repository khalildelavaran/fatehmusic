...
function mapStaticInstructor(item: any): InstructorRecord {
  ...
  return {
    ...
    firstName: String(identity.firstName || parts[0] || ""),
    lastName: String(identity.lastName || parts.slice(1).join(" ")),
...
export async function listInstructors(
  ...
  try {
    const totalSql = "SELECT COUNT(*) AS count FROM instructors " + whereSql;
    let totalStatement = db.prepare(totalSql);
    ...
    const sql =
      "SELECT " +
      COLUMNS +
      ", (SELECT COUNT(DISTINCT r.student_national_code) FROM registrations r WHERE r.instructor_id = instructors.id AND r.student_national_code IS NOT NULL AND r.student_national_code != '') AS student_count FROM instructors " +
      whereSql +
  ...
  for (const item of staticInstructors as any[]) {
    ...
    const matchesStatus =
      normalized.isActive === null ||
      item.active === normalized.isActive;
...
export async function getInstructorProfile(
  ...
  try {
    const sql = "SELECT " + COLUMNS + " FROM instructors WHERE id = ?";
    ...
    if (result && Array.isArray(result.results)) {
      for (const student of result.results) {
        students.push({
          ...
          nationalCode: String(student.national_code || ""),
          studentStatus: String(student.student_status || ""),
          course: String(student.course || ""),
          termCount: Number(student.term_count) || 0,
          startDate: String(student.start_date || ""),
          lastActivity: String(student.last_activity || "")
        });
      }
    }
  } catch (error) {
...
export function validateInstructorInput(
  ...
  if (isCreate && !input.firstName?.trim()) {
    errors.push("نام مدرس الزامی است.");
  }

  if (isCreate && !input.lastName?.trim()) {
...
export async function createInstructor(
  ...
  const sql =
    "INSERT INTO instructors (slug, first_name, last_name, phone, email, specialty, instruments, biography, notes, is_active) VALUES (?,?,?,?,?,?,?,?,?,1)";
  const statement = db.prepare(sql);

  const result: any = await statement
    .bind(

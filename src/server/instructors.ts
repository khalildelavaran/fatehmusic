...
function parseInstruments(value:any):string[] {
  ...
  try {
    ...
    var result:string[] = [];
    for (var i=0;i<parsed.length;i++) if (typeof parsed[i] === "string") result.push(parsed[i]);
    return result;
  } catch (_) { return []; }
}
...
export function normalizeInstructorListParams(params:InstructorListParams):NormalizedInstructorListParams {
  var search=String(params.search||"").trim(); var isActive:boolean|null=null; var pageNumber=Number(params.page); var sizeNumber=Number(params.pageSize); var page=1; var pageSize=DEFAULT_PAGE_SIZE;
  if(params.status==="active") isActive=true; else if(params.status==="inactive") isActive=false;
  if(Number.isFinite(pageNumber)) page=Math.max(1,Math.floor(pageNumber));
  if(Number.isFinite(sizeNumber)) pageSize=Math.min(MAX_PAGE_SIZE,Math.max(1,Math.floor(sizeNumber)));
...
export async function listInstructors(db:D1Database,params:InstructorListParams):Promise<InstructorListResult> {
  ...
  try{
    ...
    var totalRow:any=await db.prepare(totalSql).bind(...bindings).first();
    ...
    var queryBindings=bindings.slice(); queryBindings.push(normalized.pageSize,normalized.offset);
    var rows:any=await db.prepare(sql).bind(...queryBindings).all();
    if(rows&&Array.isArray(rows.results)){
      var instructors:InstructorListItem[]=[];
      for(var i=0;i<rows.results.length;i++){var instructor=mapDbInstructor(rows.results[i]);instructors.push({...instructor,studentCount:Number(rows.results[i].student_count)||0});}
  ...
  var filtered:any[]=[];
  for(var j=0;j<staticInstructors.length;j++){var item:any=staticInstructors[j];var identity=item.identity||{};var fallbackName=String(identity.firstName||"")+" "+String(identity.lastName||"");var name=String(item.name||fallbackName).trim();var position=String(item.position||"");var matchesSearch=!normalized.search||name.indexOf(normalized.search)>=0||position.indexOf(normalized.search)>=0;var matchesStatus=normalized.isActive===null||item.active===normalized.isActive;if(matchesSearch&&matchesStatus)filtered.push(item);}
  var pageItems=filtered.slice(normalized.offset,normalized.offset+normalized.pageSize);var resultInstructors:InstructorListItem[]=[];
  for(var k=0;k<pageItems.length;k++){var mapped=mapStaticInstructor(pageItems[k]);resultInstructors.push({...mapped,studentCount:0});}
  return {instructors:resultInstructors,total:filtered.length,page:normalized.page,pageSize:normalized.pageSize};
}
...
export async function getInstructorProfile(db:D1Database,id:number):Promise<InstructorProfile|null>{
  ...
  try{var statement=db.prepare("SELECT "+COLUMNS+" FROM instructors WHERE id = ?");var row:any=await statement.bind(id).first();if(row)instructor=mapDbInstructor(row);}catch(error){console.error("[admin/instructors] profile instructor query failed",error);}
  if(!instructor){for(var i=0;i<staticInstructors.length;i++){if(Number(staticInstructors[i].id)===Number(id)){instructor=mapStaticInstructor(staticInstructors[i]);break;}}}
  if(!instructor)return null;
  var students:InstructorStudentSummary[]=[];
  try{var sql="SELECT s.id AS student_id, s.first_name, s.last_name, s.national_code, s.status AS student_status, r.instrument_title AS course, COUNT(*) AS term_count, MIN(r.created_at) AS start_date, MAX(r.created_at) AS last_activity FROM registrations r JOIN students s ON s.id = r.student_id WHERE r.instructor_id = ? GROUP BY s.id, r.instrument_title ORDER BY last_activity DESC";var result:any=await db.prepare(sql).bind(id).all();if(result&&Array.isArray(result.results)){for(var j=0;j<result.results.length;j++){var studentRow=result.results[j];students.push({studentId:Number(studentRow.student_id),firstName:String(studentRow.first_name||""),lastName:String(studentRow.last_name||""),nationalCode:String(studentRow.national_code||""),studentStatus:String(studentRow.student_status||""),course:String(studentRow.course||""),termCount:Number(studentRow.term_count)||0,startDate:String(studentRow.start_date||""),lastActivity:String(studentRow.last_activity||"")});}}}catch(error){console.error("[admin/instructors] profile student query failed",error);}
...
export function validateInstructorInput(input:InstructorInput,options:InstructorValidationOptions={}):ValidationResult{
  var errors:string[]=[];var isCreate=options.isCreate===true;var firstName=typeof input.firstName==="string"?input.firstName.trim():"";var lastName=typeof input.lastName==="string"?input.lastName.trim():"";
...
export async function updateInstructor(db:D1Database,id:number,patch:InstructorInput):Promise<boolean>{
  var columnMap:{[key:string]:string}={firstName:"first_name",lastName:"last_name",phone:"phone",email:"email",specialty:"specialty",biography:"biography",notes:"notes"};
  var setClauses:string[]=[];var bindings:any[]=[];var keys=Object.keys(columnMap);
  ...
  if(result.meta&&typeof result.meta.changes==="number"&&result.meta.changes===0){
    var existing:any=await db.prepare("SELECT id FROM instructors WHERE id = ?").bind(id).first();

...
.meta-label{font-size:6.8pt;color:#777;font-weight:600}.date{font-size:8.2pt;color:#222;font-weight:850;white-space:nowrap}.tracking{font-size:7pt;font-weight:600;color:#666;white-space:nowrap}.tracking .ltr-code{direction:ltr;unicode-bidi:embed;font-weight:900;color:#876b25;letter-spacing:.2pt}
...
.footer{position:absolute;bottom:8.2mm;left:15.8mm;right:15.8mm;height:16.2mm;display:grid;grid-template-columns:2.65fr .75fr 1.1fr 1.35fr;align-items:center;direction:rtl;border-top:.4mm solid #b9973e;z-index:3}.footer:before{content:"آموزشگاه موسیقی فاتح";position:absolute;top:-3.1mm;right:50%;transform:translateX(50%);padding:0 2.5mm;background:#fff;color:#96772d;font-size:5.4pt;font-weight:800;white-space:nowrap}.footer-item{min-width:0;height:10.8mm;padding:0 2.2mm;display:flex;align-items:center;justify-content:center;gap:1.3mm;border-left:.16mm solid #ddd4bd;color:#333;font-size:5.75pt;line-height:1.35;white-space:nowrap;overflow:hidden}.footer-item:last-child{border-left:0}.footer-item:first-child{justify-content:flex-start;padding-right:1mm;padding-left:3mm}.footer-item:last-child{justify-content:flex-start;direction:ltr;gap:1.3mm;padding-left:1mm;padding-right:2.5mm}.footer-label{font-weight:900;color:#333;white-space:nowrap}.footer-value{font-weight:500;color:#666;white-space:nowrap}
...
function renderContractBody(contract: ContractResult): string {
  const articles = contract.blocks.map((block) => `<div class="contract-article">${block.heading ? `<h4>${escapeHtml(block.heading)}</h4>` : ""}${block.paragraphs.map((p) => `<p>${escapeHtml(p)}</p>`).join("")}</div>`).join("");
  const signature = `<div class="contract-signature"><div class="contract-signature-col"><span>محل امضاء هنرجو (یا ولی و سرپرست)</span><span>نام و نام خانوادگی: ${escapeHtml(contract.signature.studentName)}</span><span>تاریخ: ${escapeHtml(contract.signature.date)}</span></div><div class="contract-signature-col"><span>محل امضاء آموزشگاه</span></div></div>`;
  return articles + signature;
}

export interface ContractPdfMeta { trackingCode: string; }

export function buildContractHtml(contract: ContractResult, meta: ContractPdfMeta): string {
  const trackingLine = meta.trackingCode ? `<div class="tracking">کد پیگیری: <span class="ltr-code">${escapeHtml(meta.trackingCode)}</span></div>` : "";
  const footer = FOOTER_ITEMS.map(([label, value]) => `<div class="footer-item"><span class="footer-label">${escapeHtml(label)}</span><span class="footer-value">${escapeHtml(value)}</span></div>`).join("");

...
export const POST: APIRoute = async ({ request }) => {
  ...
  try { const body: InstructorInput = await request.json(); const validation = validateInstructorInput(body, { isCreate: true }); if (!validation.valid) return json({ success: false, message: validation.errors.join(" ") }, 422); const id = await createInstructor(db, body); await syncInstructorCourseRelations(db, id, body.instruments ?? []); const profile = await getInstructorProfile(db, id); return json({ success: true, profile }, 201); } catch (error) { console.error("[admin/instructors] create failed", error); return json({ success: false, message: "ثبت مدرس با خطای سرور مواجه شد." }, 500); }
};

export const PATCH: APIRoute = async ({ request }) => {
  const denied = await requireAdmin(request); if (denied) return denied; const db = env.DB; if (!db) return json({ success: false, message: "دیتابیس در دسترس نیست." }, 503);
  try { const body: { id?: number } & InstructorInput = await request.json(); const { id, ...patch } = body; if (!id || !Number.isInteger(id) || id <= 0) return json({ success: false, message: "شناسه مدرس معتبر نیست." }, 422); const validation = validateInstructorInput(patch, { isCreate: false }); if (!validation.valid) return json({ success: false, message: validation.errors.join(" ") }, 422); const ok = await updateInstructor(db, id, patch); if (!ok) return json({ success: false, message: "به‌روزرسانی مدرس با خطا مواجه شد." }, 500); if (patch.instruments !== undefined) await syncInstructorCourseRelations(db, id, patch.instruments); const profile = await getInstructorProfile(db, id); if (!profile) return json({ success: false, message: "مدرسی با این شناسه یافت نشد." }, 404); return json({ success: true, profile }); } catch (error) { console.error("[admin/instructors] update failed", error); return json({ success: false, message: "ذخیره دوره‌ها و اطلاعات مدرس با خطای سرور مواجه شد." }, 500); }
};

...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  const [sessions, attendance, finance] = await Promise.all([
    ...
    db.prepare(`SELECT COUNT(*) AS total, SUM(CASE WHEN es.status = 'pending' THEN 1 ELSE 0 END) AS pending FROM enrollment_sessions es JOIN class_sessions cs ON cs.id = es.session_id WHERE cs.session_date = ? AND cs.status != 'cancelled'`).bind(date).first(),
  ...
  const status: EndOfDayStatus = issues > 0 ? "needs_attention" : active === 0 ? "open" : "ready";
  ...
  return new Response(JSON.stringify({
    ...
    summary: {
      ...
      teacher_attendance_pending: teacherPending,
      student_attendance_pending: attendancePending,
      overdue_count: overdueCount,
      balance_due: balanceDue,
    },
    close_checklist: {
      teacher_attendance: teacherPending === 0,
      student_attendance: attendancePending === 0,

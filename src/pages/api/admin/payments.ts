...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!Number.isInteger(invoiceId) || invoiceId <= 0) {
      return json({ success: false, message: "صورتحساب معتبر نیست." }, 422);
    }
    if (!Number.isInteger(amount) || amount <= 0) {
      return json({ success: false, message: "مبلغ پرداخت باید یک عدد صحیح بزرگ‌تر از صفر باشد." }, 422);
    }
    ...
    if (amount > balance) {
      return json({ success: false, message: `مبلغ پرداخت نمی‌تواند بیشتر از مانده ${balance.toLocaleString("fa-IR")} باشد.` }, 422);
    }
    ...
    const payment = await db.prepare(`
      ...
      VALUES (?, ?, datetime('now'), ?, ?, ?)
      RETURNING id
    `).bind(invoiceId, amount, method || null, reference || null, note).first<{ id: number }>();

    if (!payment) return json({ success: false, message: "پرداخت ثبت نشد." }, 500);

    const newPaid = alreadyPaid + amount;
    const newStatus = newPaid >= invoice.amount
      ? "paid"
      : (invoice.due_date && invoice.due_date < todayIso() ? "overdue" : "pending");

    await db.prepare(`UPDATE invoices SET status = ?, updated_at = datetime('now') WHERE id = ?`)

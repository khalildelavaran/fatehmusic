...
export const PATCH: APIRoute = async ({ request }) => {
  try {
    ...
    const studentScheduleId = Number(body?.enrollmentSessionId);
    if (Number.isInteger(studentScheduleId) && studentScheduleId > 0 && body?.sessionId === undefined) {
      const sessionDate = body?.sessionDate;
      const startTime = body?.startTime;
      const endTime = body?.endTime;
      if (!sessionDate || !DATE_RE.test(sessionDate) || !validTime(startTime) || !validTime(endTime)) {
        return json({ success: false, message: "اطلاعات زمان‌بندی هنرجو معتبر نیست." }, 422);
      }
      if (minutes(endTime) - minutes(startTime) !== 30) {
        return json({ success: false, message: "مدت جلسه هنرجو باید دقیقاً ۳۰ دقیقه باشد." }, 422);
      }
      const studentSession = await env.DB.prepare(`
        SELECT es.id, es.session_id, cs.session_date, cs.status
        FROM enrollment_sessions es
        JOIN class_sessions cs ON cs.id = es.session_id
        WHERE es.id = ? LIMIT 1
      `).bind(studentScheduleId).first<{ id:number; session_id:number; session_date:string; status:string }>();
      if (!studentSession || studentSession.status === "cancelled" || studentSession.session_date !== sessionDate) {
      ...
      const result = await env.DB.prepare(`
        ...
        WHERE id = ?
          AND EXISTS (SELECT 1 FROM class_sessions cs WHERE cs.id = enrollment_sessions.session_id AND cs.status <> 'cancelled' AND cs.session_date = ?)
          AND NOT EXISTS (SELECT 1 FROM daily_closures dc WHERE dc.close_date = ?)
      `).bind(startTime, endTime, studentScheduleId, sessionDate, sessionDate).run();
      if (!result.meta.changes) {
        const racedClosed = await rejectIfDailyClosed(env.DB, sessionDate);
        if (racedClosed) return racedClosed;
        return json({ success:false, message:"زمان هنرجو دیگر قابل ویرایش نیست؛ اطلاعات روز را دوباره دریافت کنید." },409);
      }

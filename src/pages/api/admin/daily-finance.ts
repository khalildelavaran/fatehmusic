...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const paymentRows = await db.prepare(`
      SELECT p.id,p.amount,p.paid_at,p.method,p.reference,p.note,i.id AS invoice_id,s.id AS student_id,
        TRIM(COALESCE(s.first_name,'') || ' ' || COALESCE(s.last_name,'')) AS student_name,
        c.title AS class_title,et.id AS term_id,e.id AS enrollment_id
      ...
      WHERE date(p.paid_at)=? ORDER BY p.paid_at DESC,p.id DESC
    `).bind(date).all<{
      id:number;amount:number;paid_at:string;method:string|null;reference:string|null;note:string;
      invoice_id:number;student_id:number;student_name:string;class_title:string|null;term_id:number;enrollment_id:number;
    }>();
    ...
    const debtRows = await db.prepare(`
      SELECT i.id AS invoice_id,i.amount AS invoice_amount,i.due_date,i.status,e.id AS enrollment_id,s.id AS student_id,
        ...
        et.billing_type,et.planned_sessions,et.tuition_amount,
      ...
      WHERE i.status<>'cancelled' AND i.id=(SELECT i2.id FROM invoices i2 WHERE i2.enrollment_term_id=et.id AND i2.status<>'cancelled' ORDER BY i2.id DESC LIMIT 1)
      ORDER BY CASE WHEN i.due_date IS NULL THEN 1 ELSE 0 END,i.due_date ASC,i.id ASC
    `).bind().all<{
      invoice_id:number;invoice_amount:number;due_date:string|null;status:string;enrollment_id:number;student_id:number;
      student_name:string;class_title:string|null;billing_type:string|null;planned_sessions:number|null;tuition_amount:number|null;paid_amount:number;
    }>();

    const payments = paymentRows.results.map((row) => ({ ...row, method: normalizeMethod(row.method) }));
    const debts = debtRows.results.map((row) => {
      const finance = calculateFinance({
        invoiceAmount: Number(row.invoice_amount), paidAmount: Number(row.paid_amount), dueDate: row.due_date,
        today: date, billingType: row.billing_type, plannedSessions: row.planned_sessions,
      });
      return { ...row, balance: finance.balance, paymentStatus: finance.financialStatus, overdue: finance.overdue, nearDue: finance.nearDue };
    ...
    return json({ success:true,date,summary:{
      receivedTotal:payments.reduce((sum,row)=>sum+row.amount,0),
      outstandingTotal:debts.reduce((sum,row)=>sum+row.balance,0),
      overdueTotal:debts.filter(row=>row.overdue).reduce((sum,row)=>sum+row.balance,0),
      receivedCount:payments.length,debtorCount:debts.length,methodTotals,
    },payments,debts});
  } catch (error) {

  ...
  try {
    if (q) {
      ...
      const result = await db.prepare(`
        ...
        ORDER BY CASE result_type WHEN 'student' THEN 1 WHEN 'instructor' THEN 2 ELSE 3 END, name
        LIMIT 20
    ...
    const metrics = await db.prepare(`
      WITH daily AS (
        SELECT cs.id, cs.status, cs.instructor_id, cs.room_id, cs.start_time, cs.end_time
        FROM class_sessions cs WHERE cs.session_date = ?
      ),
      attendance AS (
        SELECT es.session_id, es.status
        FROM enrollment_sessions es JOIN daily d ON d.id = es.session_id
      ),
      financial AS (
        SELECT et.id AS term_id, et.billing_type, et.tuition_amount, et.tuition_due_date,
      ...
      SELECT
        ...
        (SELECT COUNT(*) FROM daily WHERE status <> 'cancelled' AND start_time < end_time) AS scheduled_sessions,
        ...
        (SELECT COUNT(*) FROM financial WHERE billing_type = 'monthly' OR (SELECT COALESCE(et2.planned_sessions,0) - COUNT(*) FROM enrollment_sessions es2 JOIN enrollment_terms et2 ON et2.id = es2.enrollment_term_id WHERE et2.id = financial.term_id AND es2.status IN ('present','absent')) <= 1) AS renewal_candidates
    ...
    const instructors = await db.prepare(`
      SELECT cs.instructor_id,
        ...
        MAX(cs.end_time) AS last_session
      ...
      GROUP BY cs.instructor_id, instructor_name
    ...
    return json({
      success: true,
      date,
      metrics: { ...(metrics ?? {}), paymentsToday: Number(payments?.amount ?? 0), paymentCountToday: Number(payments?.count ?? 0) },

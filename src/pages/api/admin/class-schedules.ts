...
export async function createClassSchedule(db: D1Database, input: ClassScheduleInput): Promise<number> {
  ...
  const classRow = await db.prepare("SELECT id, class_type FROM classes WHERE id = ?").bind(input.classId).first<{ id: number; class_type: string }>();
  ...
  // class's weekly slot is always exactly 30 minutes long.
  const isIndividual = classRow.class_type !== "group" && classRow.class_type !== "workshop";
  if (isIndividual) {
    const durationMinutes = timeToMinutes(input.endTime) - timeToMinutes(input.startTime);
    if (durationMinutes !== 30) throw new Error("مدت جلسه کلاس‌های تکی باید دقیقاً ۳۰ دقیقه باشد.");
  }

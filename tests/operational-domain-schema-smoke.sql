...
SELECT CASE
  WHEN (SELECT COUNT(*) FROM sqlite_master WHERE type='trigger' AND name IN (
    'trg_class_sessions_makeup_insert', 'trg_class_sessions_makeup_update',
    'trg_enrollment_sessions_validate_insert', 'trg_enrollment_sessions_validate_update',
    'trg_enrollment_sessions_makeup_insert', 'trg_enrollment_sessions_makeup_update',
    'trg_teacher_attendance_validate_insert', 'trg_teacher_attendance_validate_update'
  )) = 8
